...
#
    ...
    def connect(self):
        ...
        try:
            ...
            logger.error("LGSmartTV: Verbindung zu {} nicht möglich!".format(self._host))
        
        logger.debug("LGSmartTV: Sending WS handshake: ", format(ws_handshake_cmd))
        response = self.send(ws_handshake_cmd, "ws")

        # Empfangenes prüfen und Key suchen
        if response == '':
            logger.error("LGSmartTV: ERROR during WS handshake!")
            returnkey = ''
        else:
            logger.debug("LGSmartTV: WS Handshake Response: {}",format(response.decode('ascii')))
        ...
        if returnkey != pdigest:
            ...
            logger.debug("LGSmartTV: Successfull WS connection to : {0} : {1}",format(self._host,self._port))
        return self._connected
    def disconnect(self):
        self._connected = False
        self._sock.close()
        logger.debug("LGSmartTV: Connection closed to {}", format(self._host))
        return None
    ...
    def lg_handshake(self):
        if self._connected == False:
            ...
            if self._tvkey != '':  #handshakecode vorhanden
                ...
                handshake = "{\"type\":\"register\",\"id\":\"register_0\",\"payload\":{\"forcePairing\":false,\"pairingType\":\"PROMT\",\"manifest\":{\"manifestVersion\":1,\"appVersion\":\"1.1\",\"signed\":{\"created\":\"20140509\",\"appId\":\"com.lge.test\",\"vendorId\":\"com.lge\",\"localizedAppNames\":{\"\":\"LG Remote App\",\"ko-KR\":\"리모컨 앱\",\"zxx-XX\":\"ЛГ Rэмotэ AПП\"},\"localizedVendorNames\":{\"\":\"LG Electronics\"},\"permissions\":[\"TEST_SECURE\",\"CONTROL_INPUT_TEXT\",\"CONTROL_MOUSE_AND_KEYBOARD\",\"READ_INSTALLED_APPS\",\"READ_LGE_SDX\",\"READ_NOTIFICATIONS\",\"SEARCH\",\"WRITE_SETTINGS\",\"WRITE_NOTIFICATION_ALERT\",\"CONTROL_POWER\",\"READ_CURRENT_CHANNEL\",\"READ_RUNNING_APPS\",\"READ_UPDATE_INFO\",\"UPDATE_FROM_REMOTE_APP\",\"READ_LGE_TV_INPUT_EVENTS\",\"READ_TV_CURRENT_TIME\"],\"serial\":\"2f930e2d2cfe083771f68e4fe7bb07\"},\"permissions\":[\"LAUNCH\",\"LAUNCH_WEBAPP\",\"APP_TO_APP\",\"CLOSE\",\"TEST_OPEN\",\"TEST_PROTECTED\",\"CONTROL_AUDIO\",\"CONTROL_DISPLAY\",\"CONTROL_INPUT_JOYSTICK\",\"CONTROL_INPUT_MEDIA_RECORDING\",\"CONTROL_INPUT_MEDIA_PLAYBACK\",\"CONTROL_INPUT_TV\",\"CONTROL_POWER\",\"READ_APP_STATUS\",\"READ_CURRENT_CHANNEL\",\"READ_INPUT_DEVICE_LIST\",\"READ_NETWORK_STATE\",\"READ_RUNNING_APPS\",\"READ_TV_CHANNEL_LIST\",\"WRITE_NOTIFICATION_TOAST\",\"READ_POWER_STATE\",\"READ_COUNTRY_INFO\"],\"signatures\":[{\"signatureVersion\":1,\"signature\":\"eyJhbGdvcml0aG0iOiJSU0EtU0hBMjU2Iiwia2V5SWQiOiJ0ZXN0LXNpZ25pbmctY2VydCIsInNpZ25hdHVyZVZlcnNpb24iOjF9.\"}]}}}"
                logger.debug("LGSmartTV: Sending LG handshake:{}", format(handshake)) 
            response = self.send(handshake, '')
            print('RAW response nach handshake', response)

            if response != '':
                result = self.json_string(response)
                logger.debug("LGSmartTV: Handshake Response:{}", format(result)) 
                if 'id' in result.keys() and result['id'] == 'result_0' and 'client-key' in result['payload'].keys():
                    ...
                    if self._lg_key == result['payload']['client-key']:
                        logger.debug("LGSmartTV: LG Client-Key successfully approved") 
                    ...
                    while (self.timestopp()- start < 60 and not lg_key_received and not error_received):
                        ...
                        logger.debug("LGSmartTV: HANDSHAKE RESPONSE ", format(response)) 
                        result = self.json_string(response)
                        if 'id' in result and result['id'] == 'register_0' and ('client-key' in result['payload']):
                            ...
                            logger.debug("LGSmartTV: LG Client-Key successfully received: {}", format(self._lg_key))
                        elif ('id' in result) and result['id'] == 'register_0' and ('error' in result):
                            error_received = True
                            logger.error("LGSmartTV: ERROR: {}", format(result['error']))
            else:
                logger.error("ERROR during LG handshake:")
    #
        ...
        if self._connected == False:
            ...
            if (response):
                ...
                if "error" in responsedict.keys():
                    logger.error("LGSmartTV: ERROR: {0}, by id {1}", format(responsedict['error'],responsedict['id']))
                    logger.error("LGSmartTV: by this Command:{}", format(cmd))
            else:
                logger.error("LGSmartTV: No Data Recieved! Error sending command:{}", format(cmd))
            return response
